...
export default function AgentStatus({ agents: propAgents }: AgentStatusProps) {
  ...
  return (
        <div className="space-y-4">
          {agents.map((agent) => (
                  <div key={agent.id} className="bg-white/5 backdrop-blur-sm rounded-xl border border-white/10 p-6 text-white">
                            <div className="flex items-center justify-between">
                                        <div className="flex items-center space-x-4">
                                                      ...
                                                      <div>
                                                                      <h3 className="font-semibold">{agent.name}</h3>h3>
                                                                      <p className="text-sm text-gray-400">{agent.type}</p>p>
                                                      </div>div>
                                        </div>div>
                            ...
                            <div className="mt-4 grid grid-cols-2 gap-4 text-sm">
                                        <div className="flex items-center space-x-2 text-gray-400">
                                                      ...
                                                      <span>{agent.performance}% performance</span>span>
                                        </div>div>
                                        <div className="flex items-center space-x-2 text-gray-400">
                                                      <CheckCircle className="w-4 h-4" />
                                                      <span>{agent.tasksCompleted} tasks</span>span>
                                        </div>div>
                            </div>div>
                  </div>div>
                ))}
